...
# SLURM script for workflow experiment with RAG simulator (no API calls needed)

#SBATCH --job-name=CARDS-WORKFLOW-RAG
#SBATCH --nodes=1
...
#SBATCH --output=slurm_logs/workflow_rag_%j.out
#SBATCH --error=slurm_logs/workflow_rag_%j.err
...
USER_SIMULATOR="rag"  # RAG simulator (no API needed)
MAX_EXAMPLES=500  # Can run more examples since no API calls
...
# Run workflow experiment with RAG simulator
