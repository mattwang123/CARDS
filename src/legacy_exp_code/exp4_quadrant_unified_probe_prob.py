"""
================================================================================
EXPERIMENT 4: Quadrant Dynamics (Unified Geometric Frame)
================================================================================
Leverages a SINGLE frozen, unified probe (from Exp 5) across all timesteps.
- Evaluates true geometric wandering from a stationary "Truth" axis.
...
- Evaluates the EOS token on the exact same unified hyperplane.
================================================================================
"""
...
def main():
    ...
    print(f"=== Running Unified Quadrant Dynamics for {args.dataset.upper()} ===\n")
    
    exp5_results_path = os.path.join(RESULTS_DIR, f"exp5_global_dynamics_{args.dataset}.json")
    
    if not os.path.exists(exp5_results_path):
        print(f"Error: Run Exp 5 Unified Probe first. Missing {exp5_results_path}")
        return
    ...
    for model_name in MODELS:
        ...
        # 4. Quadrant-Specific Temporal Tracking (Unified Geometric Frame)
        print("\n[2] Unified Quadrant Tracking (Active-Only Mean Probability)")
        ...
        # Identify the Unified Layer from Exp 5
        if model_name not in exp5_results or "unified_layer" not in exp5_results[model_name]:
            print(f"   -> Missing Unified Layer data for {model_name}. Skipping.")
            continue
            
        unified_layer = exp5_results[model_name]["unified_layer"]
        # Make sure this filename matches how you saved your Unified Probe in Exp 5
        probe_path = os.path.join(probes_dir, f"unified_probe_layer{unified_layer}.joblib")
        
        if not os.path.exists(probe_path):
            print(f"   -> Unified Probe missing at {probe_path}")
            continue
        ...
        for t in TIMESTEPS:
            ...
            # CRITICAL: Always slice at the unified layer to maintain the stationary geometric frame
            X_test_layer = X_test_all[:, unified_layer, :]
            
            if len(X_test_layer) != len(quadrants):
                print(f"   -> Data mismatch! Embeddings: {len(X_test_layer)}, Quadrants: {len(quadrants)}")
                break
            
            # Predict probability along the stationary unified axis
            ...
            print(f"   t={str(t):<3} | Unified Lyr: {unified_layer:<2} | Q1: {fmt(p_q1)} (n={n_q1:<3}) | Q2: {fmt(p_q2)} (n={n_q2:<3}) | Q3: {fmt(p_q3)} (n={n_q3:<3}) | Q4: {fmt(p_q4)} (n={n_q4:<3})")
    ...
    csv_out = os.path.join(RESULTS_DIR, f'exp4_unified_quadrant_dynamics_{args.dataset}.csv')
    ...
    json_out = os.path.join(RESULTS_DIR, f'exp4_unified_quadrant_dynamics_{args.dataset}.json')
    with open(json_out, 'w') as f:
        json.dump(json_dict, f, indent=2)
        
    print(f"\n[SUCCESS] Unified Quadrant Data saved to:\n -> {csv_out}\n -> {json_out}")
