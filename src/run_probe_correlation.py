...
2. Trains probes on best layers (default) or multiple layer groups (for H3)
...
def evaluate_layer_groups_for_h3(model_name, dataset_name, device, pooling, train_questions, train_labels, test_questions, test_labels):
    """
    Evaluate early, middle, late layer groups for H3 analysis
    
    Returns:
        dict: Results for each layer group with sample-level predictions
    """
    print(f"  → Evaluating layer groups for H3...")
    ...
    # Define layer groups (early: 0-33%, middle: 33-66%, late: 66-100%)
    layer_groups = {
    ...
    for group_name, layers in layer_groups.items():
        print(f"    Testing {group_name} layers: {layers}")
        ...
        # Find best layer within this group
        for layer_idx in layers:
            ...
            # Evaluate
            metrics, predictions = evaluate_probe_with_predictions(
        ...
        print(f"    Best {group_name} layer: {best_layer} (F1 = {best_f1:.4f})")
        
        results[group_name] = {
            'best_layer': best_layer,
            'layer_range': layers,
            'metrics': best_metrics,
            'detailed_predictions': best_predictions
        }
...
def evaluate_model_on_dataset(model_name, dataset_name, device, pooling, output_dir, 
                               probe_results_dir, test_mode=False, test_samples=3, layer_analysis=False):
    """Evaluate probe on best layer or multiple layer groups with sample-level predictions"""
    print(f"\n{'='*80}")
    ...
    if layer_analysis:
        # H3: Multi-layer analysis
        print(f"Running layer-wise analysis for H3...")
        layer_results = evaluate_layer_groups_for_h3(
        ...
        final_results = {
            ...
            'layer_groups': layer_results
        }
        
        # Save layer analysis results
        if not test_mode:
            results_path = os.path.join(output_dir, f"{model_name}_{dataset_name}_layer_analysis.json")
            with open(results_path, 'w') as f:
                json.dump(final_results, f, indent=2)
            print(f"\n✓ Saved layer analysis: {results_path}")
        
        # Print layer comparison
        print(f"\n{'='*80}")
        print(f"LAYER ANALYSIS: {model_name} on {dataset_name}")
        print(f"{'='*80}")
        for group_name, group_results in layer_results.items():
            metrics = group_results['metrics']
            print(f"{group_name.upper()} layers (best: {group_results['best_layer']}):")
...
def run_test_mode(device, pooling, probe_results_dir, layer_analysis, test_samples=3):
    ...
    result = evaluate_model_on_dataset(
        ...
        test_mode=True, test_samples=test_samples, layer_analysis=layer_analysis
    )
    ...
    print(f"{'='*80}")
    if layer_analysis:
        print(f"Layer groups evaluated: {list(result['layer_groups'].keys())}")
        for group_name, group_result in result['layer_groups'].items():
            print(f"  {group_name}: layer {group_result['best_layer']}, "
                  f"F1 = {group_result['metrics']['f1']:.4f}")
    else:
...
def main():
    ...
    parser.add_argument('--layer_analysis', action='store_true',
                        help='Evaluate multiple layer groups for H3 analysis')
    ...
    # Test mode
    if args.test:
        run_test_mode(args.device, args.pooling, args.probe_results_dir, args.layer_analysis, args.test_samples)
        return
    ...
    print(f"Layer analysis: {args.layer_analysis}")
    print("="*80)
    ...
    for model_name in MODELS:
        ...
        for dataset_name in DATASETS.keys():
            result = evaluate_model_on_dataset(
                ...
                args.output_dir, args.probe_results_dir, layer_analysis=args.layer_analysis
            )
            ...
            if args.layer_analysis:
                ...
                model_results[dataset_name] = {
                    ...
                    'layer_groups': {
                        group_name: {
                            'best_layer': group_result['best_layer'],
                            'accuracy': group_result['metrics']['accuracy'],
                            'f1': group_result['metrics']['f1']
                        }
                        for group_name, group_result in result['layer_groups'].items()
                    }
                }
            else:
                # Store single layer results
                model_results[dataset_name] = {
                    'mode': 'single_layer',
    ...
    # Save summary
    summary_filename = 'layer_analysis_summary.json' if args.layer_analysis else 'probe_correlation_summary.json'
    ...
    if args.layer_analysis:
        ...
        print(f"{'='*80}")
        print(f"\n{'Model':<25} {'Dataset':<10} {'Early':<8} {'Middle':<8} {'Late':<8}")
        print("-" * 65)

        for model_name in MODELS:
            for dataset_name in ['umwp', 'gsm8k', 'treecut']:
                layer_groups = all_results[model_name][dataset_name]['layer_groups']
                early_f1 = layer_groups['early']['f1']
                middle_f1 = layer_groups['middle']['f1']
                late_f1 = layer_groups['late']['f1']
                
                print(f"{model_name[:24]:<25} {dataset_name:<10} {early_f1:.3f}    {middle_f1:.3f}     {late_f1:.3f}")
    else:
